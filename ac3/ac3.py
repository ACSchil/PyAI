    ...
    for x in dom1:
        ...
        for y in dom2:
            if bc.func(x,y):
                remove = False
                break
        if remove and len(bc.var1.domain) > 1:
...
def tryAC3():
    ...
    constraints.append(BinaryConstraint(variables['A1'], variables['A2'], lambda x, y: abs(x-y) == 2 or abs(y-x) == 2))
    constraints.append(BinaryConstraint(variables['A2'], variables['A1'], lambda x, y: abs(x-y) == 2 or abs(y-x) == 2))
    constraints.append(BinaryConstraint(variables['B1'], variables['C1'], lambda x, y: (x/y) == 2 or (y/x) == 2))
    constraints.append(BinaryConstraint(variables['C1'], variables['B1'], lambda x, y: (x/y) == 2 or (y/x) == 2))
    constraints.append(BinaryConstraint(variables['B2'], variables['B3'], lambda x, y: (x/y) == 3 or (y/x) == 3))
    constraints.append(BinaryConstraint(variables['B3'], variables['B2'], lambda x, y: (x/y) == 3 or (y/x) == 3))
    constraints.append(BinaryConstraint(variables['C2'], variables['C3'], lambda x, y: abs(x-y) == 1 or abs(y-x) == 1))
    constraints.append(BinaryConstraint(variables['C3'], variables['C2'], lambda x, y: abs(x-y) == 1 or abs(y-x) == 1))
    allDiff( constraints, [variables['A1'], variables['A2'], variables['A2']] )
